...
namespace PE
{
	...
	namespace Components
	{
		...
		std::unordered_map<int, Ghost *> ServerNetworkManager::ghostMap;
		...
		void ServerNetworkManager::createNetworkConnectionContext(t_socket sock, int clientId, PE::NetworkContext *pNetContext)
		{
			...
			{
				pNetContext->m_pStreamManager = new (m_arena) StreamManager(*m_pContext, m_arena, *pNetContext, Handle(), true);
				pNetContext->getStreamManager()->addDefaultComponents();
			}

			{
				pNetContext->m_pEventManager = new (m_arena) EventManager(*m_pContext, m_arena, *pNetContext, Handle(), true);
				pNetContext->getEventManager()->addDefaultComponents();
			}

			{
				pNetContext->m_pGhostManager = new (m_arena) GhostManager(*m_pContext, m_arena, *pNetContext, Handle(), true);
		...
		void ServerNetworkManager::debugRender(int &threadOwnershipMask, float xoffset /* = 0*/, float yoffset /* = 0*/)
		{
			...
			for (unsigned int i = 0; i < m_clientConnections.m_size; ++i)
			{
				...
				// DEBUG RENDER EVENT MANAGER LOOK AT CLIENT FOR OTHER
				//DebugRenderer::Instance()->createTextMesh(
				//	PEString::s_buf, true, false, false, false, 0,
			...
			for (auto it = GhostManager::serverGhostMap.begin(); it != GhostManager::serverGhostMap.end(); it++)
			{
				...
				if (ghost->type == Ghost::SoldierNPC)
				{
					...
					sprintf(PEString::s_buf, "Server");
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, false, true, true, false, 0,
						base.getPos(), 1.0f, threadOwnershipMask);
				}
			}
		...
		void ServerNetworkManager::scheduleEventToAllExcept(PE::Networkable *pNetworkable, PE::Networkable *pNetworkableTarget, int exceptClient, bool shouldGaurantee)
		{
			for (unsigned int i = 0; i < m_clientConnections.m_size; ++i)
			{
				...
				NetworkContext &netContext = m_clientConnections[i];
				netContext.getEventManager()->scheduleEvent(pNetworkable, pNetworkableTarget, shouldGaurantee);
			}
		}

		void ServerNetworkManager::scheduleGhostToAll(Ghost *pGhost)
		{
			for (unsigned int i = 0; i < m_clientConnections.m_size; ++i)
			{
				NetworkContext &netContext = m_clientConnections[i];
				netContext.getGhostManager()->scheduleGhost(pGhost);
			}
		}
