...
t_socket ServerConnectionManager::udpSocket = INVALID_SOCKET;
int ServerConnectionManager::udpPort = -1;
...
: ConnectionManager(context, arena, netContext, hMyself)
{
	...
	{
		err = inet_trycreate(&ServerConnectionManager::udpSocket, SOCK_DGRAM);
		if (err)
		{
			assert(!"error creating server udp socket occurred");
		}

		err = inet_trybind(&ServerConnectionManager::udpSocket, "0.0.0.0", port); // leaves socket non-blocking
		udpPort = port;
	}
}
