...
namespace PE
{
	namespace Components
	{
		...
		bool ClientNetworkManager::showClientDraw;
		void ClientNetworkManager::debugRender(int &threadOwnershipMask, float xoffset /* = 0*/, float yoffset /* = 0*/)
		{
			...
			if (showClientDraw)
			{
				if (m_netContext.getGhostManager() != NULL && m_netContext.getGhostManager()->ghostMap.find(CharacterControl::Components::ClientGameObjectManagerAddon::clientID + 1000) != m_netContext.getGhostManager()->ghostMap.end())
				{
					Matrix4x4 base;
					Ghost *ghost = m_netContext.getGhostManager()->ghostMap[CharacterControl::Components::ClientGameObjectManagerAddon::clientID + 1000];
