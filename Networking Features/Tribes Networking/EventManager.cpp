...
namespace PE
{
	namespace Components
	{
		...
		EventManager::EventTransmissionRecord EventManager::SendingSlidingWindow[EventManager::PE_EVENT_SLIDING_WINDOW];
		int EventManager::SendingSlidingWindowStart = 1;
		int EventManager::SendingSlidingWindowSize = 0;
		bool EventManager::READYTOGO = false;

		EventManager::EventManager(PE::GameContext &context, PE::MemoryArena arena, PE::NetworkContext &netContext, Handle hMyself, bool isServer)
		...
		void EventManager::scheduleEvent(PE::Networkable *pNetworkableEvent, PE::Networkable *pNetworkableTarget, bool guaranteed)
		{
			...
			if (READYTOGO && !m_isServer && guaranteed) // guaranteed event
			{
				EventTransmissionRecord record;
				record.classID = classId;
				record.orderID = back.m_orderId;
				record.LastTimeSent = TIMENOW;
				memcpy(&record.data, &back, sizeof(EventTransmissionData));
				record.data.m_size = dataSize;
				SendingSlidingWindow[record.orderID - EventManager::SendingSlidingWindowStart] = record;
				SendingSlidingWindowSize++;
				char buf[256];
				sprintf(buf, "sent event %d ... sliding window is now of size: %d\n", record.orderID, SendingSlidingWindowSize);
		...
		int EventManager::fillInNextPacket(char *pDataStream, TransmissionRecord *pRecord, int packetSizeAllocated, bool &out_usefulDataSent, bool &out_wantToSendMore)
		{
			...
			StreamManager::WriteInt32(eventsReallySent, &pDataStream[0]);
			...
			for (int i = 0; i < numEvents; ++i)
			{
				...
				if (evtOrderId > 0)
				{
					...
					if (indexInEventsArray < 0)
					{
						...
						if (pEvt->m_networkClientId != -1)
						{
							CharacterControl::Components::ServerGameObjectManagerAddon::needToAck.push_back(std::pair<int, int>(evtOrderId, pEvt->m_networkClientId));
						}
