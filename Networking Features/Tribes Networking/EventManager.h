...
struct EventManager : public Component
{
	...
	// Constructor -------------------------------------------------------------
	EventManager(PE::GameContext &context, PE::MemoryArena arena, PE::NetworkContext &netContext, Handle hMyself, bool isServer = false);
	...
	struct EventTransmissionRecord
	{
		...
		bool isAcked = false;
		EventTransmissionData data;
	};

	bool m_isServer;
	static EventTransmissionRecord SendingSlidingWindow[PE_EVENT_SLIDING_WINDOW];
	static int SendingSlidingWindowStart;
	static int SendingSlidingWindowSize;
	static bool READYTOGO;
