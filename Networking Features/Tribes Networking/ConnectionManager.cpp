...
//#define IAMSERVER
//#define IAMCLIENT
// change IP of other target
#define OTHERADDRESS "192.168.43.49"
...
namespace PE
{
	namespace Components
	{
		...
		int ConnectionManager::staticClientID = 0;
		void ConnectionManager::initializeConnected(t_socket sock)
		{
			...
			if (m_isServer)
			{
				...
				if (clientID == 0)
				{
					socket_setnonblocking(&ServerConnectionManager::udpSocket);
				}
			}
			else
			{
				socket_setnonblocking(&ClientConnectionManager::udpSocket);
			}
		}
		...
		void ConnectionManager::disconnect()
		{
			...
			if (!m_isServer)
			{
				socket_destroy(&ClientConnectionManager::udpSocket);
			}
			else
			{
				socket_destroy(&ServerConnectionManager::udpSocket);
			}
		}
...
//#define UDP
		void ConnectionManager::sendPacket(Packet *pPacket, TransmissionRecord *pTransmissionRecord)
		{
			...
			StreamManager::ReadInt32(&pPacket->m_data[0], packetSize);
			...
			while (total < count && err == IO_DONE)
			{
...
#ifdef IAMCLIENT
				inet_pton(AF_INET, OTHERADDRESS, &serverHint.sin_addr);
#endif
...
#ifdef IAMLOCAL
					serverHint.sin_port = htons(54000 + clientID);
#endif
					err = socket_sendto(&ServerConnectionManager::udpSocket, &pPacket->m_data[total], step, &done, (sockaddr *)&serverHint, sizeof(serverHint), tm);
					static int count = 0;
					count++;
					if (count % 5 == 0)
					{
						char buf[256];
						sprintf(buf, "server has sent %d packets\n", count);
				...
				else
				{
					...
					err = socket_sendto(&ClientConnectionManager::udpSocket, &pPacket->m_data[total], step, &done, (sockaddr *)&serverHint, sizeof(serverHint), tm);
					static int count = 0;
					count++;
					if (count % 5 == 0)
					{
						char buf[256];
						sprintf(buf, "client has sent %d packets\n", count);
						OutputDebugStringA(buf);
					}
				}
			...
			// in future we will use UDP with actual ACK packets
		...
		void ConnectionManager::receivePackets()
		{
			...
			while (true)
			{
				...
				if (m_isServer)
				{
					...
					err = socket_recvfrom(&ServerConnectionManager::udpSocket, &m_buffer[m_bytesBuffered], PE_SOCKET_RECEIVE_BUFFER_SIZE - m_bytesBuffered, &got, (sockaddr *)&serverHint, &serverLen, tm);
					//char buf[256];
				...
				{
					serverHint.sin_port = htons(44000);
					err = socket_recvfrom(&ClientConnectionManager::udpSocket, &m_buffer[m_bytesBuffered], PE_SOCKET_RECEIVE_BUFFER_SIZE - m_bytesBuffered, &got, (sockaddr *)&serverHint, &serverLen, tm);
				}
#endif
